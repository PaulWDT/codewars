...
unmarked numbers on that board; in this case, the sum is 188. Then, multiply that sum by the number that was just called when the board won, 24, to get the final score, 188 * 24 = 4512.
...
func main() {
	...
	for scanner.Scan() {
		...
		if inBoardStringIdx < 5 { // always read 5 lines into the array "inBoardStrings"
			inBoardStrings[inBoardStringIdx] = lineContent
			inBoardStringIdx++
		}
		if inBoardStringIdx == 5 { // once we reach the 5th line of strings -> convert the 5 strings into a board - a 5x5 array of ints
			board = stringsToBoard(inBoardStrings)
			boards = append(boards, board) // and add the board to the slice of all bingo boards
			boardNumber++
	...
	// solve bingo ... : call bingo with each int in inValues and the pointer to the slice of bingo-boards
	// bingo() will detect when the first board is solved.
...
func bingo(boards *[]boardType, number int) {
	//	fmt.Printf("len boards ? %T,%d", boards, len(*boards))
	for boardIdx := 0; boardIdx < len(*boards); boardIdx++ {
		elimNumInBoard(&(*boards)[boardIdx], number) // call func with the pointer to the board with "boardIdx" inside the global slice "boards"

		if boardIdx == 0 {
			fmt.Println("display board at boardIndex = ", boardIdx, " board = ", (*boards)[boardIdx])
		}
		win := checkBoardBingo(&(*boards)[boardIdx])
		if win {
			fmt.Println("Found the winning board at boardIndex = ", boardIdx, " board = ", (*boards)[boardIdx])

			fmt.Println("Score of winning board = ", calcScore(&(*boards)[boardIdx], number))
	...
	/*	for boardIdx, board := range *boards {
			...
			//	fmt.Printf("what is board ? %T", &(boards[boardIdx]))
...
func calcScore(board *boardType, number int) (result int) {
	for lineIdx, line := range board {
		for rowIdx, _ := range line {
			result += board[lineIdx][rowIdx]
		}
	}
